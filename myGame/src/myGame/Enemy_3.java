...
public class Enemy_3 extends Enemy {
	...
	public Enemy_3(World newWorld, Character newCharacter, float newX, float newY) {
	...
	public void loadDData(){
		try{
			enemySpaceShip = TextureLoader.getTexture("png", ResourceLoader.getResourceAsStream("res/enemyShip_2.png"), GL11.GL_NEAREST);
		}
		catch(IOException e){
			e.printStackTrace();
		}
		...
		y += yVel * delta * 0.15;                
			...
			for(int j = 0; j < gameWorld.projectiles.size(); j++){
				...
				if(gameWorld.projectiles.get(j).x > x && gameWorld.projectiles.get(j).x + 1 < x + 22 && gameWorld.projectiles.get(j).y > y && gameWorld.projectiles.get(j).y + 1 < y + 22){
					gameWorld.enemies_3.remove(this);
					playSound(hitSound);
